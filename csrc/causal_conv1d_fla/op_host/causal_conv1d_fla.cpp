...
void CopyTilingToDevice(const CausalConv1dTilingData &td, at::Tensor &tiling_tensor, const at::Tensor &x)
{
    ...
    const size_t aligned_bytes =
        (tiling_bytes + static_cast<size_t>(PADDING_BYTE) - 1U) / static_cast<size_t>(PADDING_BYTE) *
        static_cast<size_t>(PADDING_BYTE);

    tiling_tensor = at::empty({static_cast<int64_t>(aligned_bytes)}, x.options().dtype(at::kByte));
    const aclError ret = aclrtMemcpy(tiling_tensor.data_ptr<uint8_t>(), aligned_bytes, &td, tiling_bytes,
                                     ACL_MEMCPY_HOST_TO_DEVICE);
    TORCH_CHECK(ret == ACL_SUCCESS, "aclrtMemcpy for tiling data failed with code ", static_cast<int>(ret));
}
                                                   ...
                                                   bool activation_mode, int64_t pad_slot_id)
{
    ...
    // Choose dim tiling plan
    DimTileChoice baseDimChoice;
    int64_t fnPlan;
    if (dim <= MAX_DIM_TILE_SIZE) {
        baseDimChoice = ChooseFnTokenFirstBaseDimChoice(dim);
        fnPlan = FN_EXECUTION_PLAN_CUTBS;
    } else {
        baseDimChoice = ChooseFnTokenDimCoSplitBaseDimChoice(dim, ubSize, core_num);
    ...
    if (inputMode == 0 && tokenChoice.tokenBlockCnt <= 128) {
    ...
    int64_t effectiveGridSize = tokenChoice.gridSize;
    int32_t block_dim =
        static_cast<int32_t>(std::min<int64_t>(effectiveGridSize, std::min<int64_t>(coreMapping.blockDim, core_num)));
    if (block_dim <= 0) {
        block_dim = 1;
    }
    ...
    CopyTilingToDevice(td, tiling_tensor, x);
